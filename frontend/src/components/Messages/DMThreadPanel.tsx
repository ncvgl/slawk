...
import { getDMThread, replyToDM, uploadFile, getUsers, type ApiDirectMessage, type ApiFile, type AuthUser } from '@/lib/api';
...
function transformDM(dm: ApiDirectMessage): DMThreadMessage {
  return {
    id: dm.id,
    content: dm.content,
...
export function DMThreadPanel({ dmId, onClose, onReplyCountChange }: DMThreadPanelProps) {
  const [parentMessage, setParentMessage] = useState<DMThreadMessage | null>(null);
  const [replies, setReplies] = useState<DMThreadMessage[]>([]);
  ...
  useEffect(() => {
    ...
    getDMThread(dmId)
      .then((data) => {
        if (cancelled) return;
        setParentMessage(transformDM(data.parent));
        setReplies(data.replies.map(transformDM));
      })
      .catch(() => {
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        {isLoading ? (
          ...
          <div data-testid="dm-thread-load-error" className="text-center text-sm text-slack-error py-4">{loadError}</div>
        ) : (
          <>
            {/* Parent message */}
            {parentMessage && (
              <div className="mb-4 pb-3 border-b border-slack-border">
                <div className="flex items-start gap-2">
                  <Avatar
                    src={parentMessage.fromUser.avatar ?? undefined}
                    alt={parentMessage.fromUser.name}
                    fallback={parentMessage.fromUser.name}
                    size="md"
                    className="mt-0.5"
                  />
            ...
            {replies.map((reply) => (
              <div key={reply.id} className="mb-3">
                <div className="flex items-start gap-2">
                  <Avatar
                    src={reply.fromUser.avatar ?? undefined}
                    alt={reply.fromUser.name}
                    fallback={reply.fromUser.name}
                    size="sm"
                    className="mt-0.5"
                  />
                  <div className="flex-1 min-w-0">
      ...
      <div className="relative border-t border-slack-border px-4 py-3">
        {replyError && (
          <p data-testid="dm-thread-reply-error" className="mb-2 text-xs text-slack-error">{replyError}</p>
        )}
        {uploadError && (
          <p className="mb-2 text-xs text-slack-error">{uploadError}</p>
        )}
        <div data-testid="dm-thread-reply-input" className="slawk-editor rounded-[8px] border border-slack-border-light">
          {/* Formatting Toolbar */}
          ...
          <div className="flex items-center justify-between px-[6px] py-1">
            <div className="flex items-center">
              <Button
                data-testid="dm-thread-attach-file-button"
                variant="toolbar"
                size="icon-sm"
              ...
              <Button
                data-testid="dm-thread-mention-button"
                variant="toolbar"
                size="icon-sm"
              ...
              {isRecording ? (
                <div className="flex items-center gap-1.5 ml-1">
                  ...
                  <Button
                    data-testid="dm-thread-mic-stop-button"
                    variant="toolbar"
                    size="icon-sm"
                ...
                <Button
                  data-testid="dm-thread-mic-button"
                  variant="toolbar"
                  size="icon-sm"
            ...
            <button
              data-testid="dm-thread-send-button"
